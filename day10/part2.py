...
def build_path(maze, start, direction):
    pos_y, pos_x = start
    pipe = None
    path = [(pos_y, pos_x)]
    while pipe != START:
...
def find_tiles_inside_path(maze, path):
    polygon = Polygon(path)
    tiles = []
    for pos_y in range(len(maze)):
        for pos_x in range(len(maze[0])):
            if (pos_y, pos_x) not in path:
                point = Point(pos_y, pos_x)
...
if __name__ == "__main__":
    for filename in sys.argv[1:]:
        ...
        path = build_path(maze, start, direction)
        tiles = find_tiles_inside_path(maze, path)
        print(f"Tiles enclosed by loop is {len(tiles)} in file {filename}")
